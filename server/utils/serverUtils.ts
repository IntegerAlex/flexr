...
export async function addNginxLocationBlock(port: number, locationPath: string): Promise<void> {
    try {
...
`;
        ...
        serverBlocks.forEach((block, index) => {
            if (block.includes('listen 443 ssl') && block.includes('server_name sites.flexhost.tech')) {
                // Check if location block already exists
                if (!block.includes(`location ${locationPath} {`)) {
                    // Append the new location block
                    const updatedBlock = block.replace(/}(?=[^}]*$)/, `${newLocationBlock}}`);
